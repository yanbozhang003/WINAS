 ...
 */
    ...
    if((sock = (int)socket(AF_INET,SOCK_STREAM,0)) == -1){	//descriptor for the new socket returned if successful
        perror("socket"); 
        return 0;
    } 
 ...
 */
    ...
    for(SW_idx=1;SW_idx<=16;SW_idx++){		//switch between 4 RFs
	    RF_SW_IDX = SW_idx & 0xFF;
	    printf("debug: write switch number!\n");
	    ret = write(fd_usb,&RF_SW_IDX,1);	//select RF by USB
	    if(!ret){
	    	printf("Warning: bytes sent to USB failed!\n");
	    }

	    // File name: /tmp/log_RF_switch_1_TraceIndex_1.txt
	    //printf("debug: create empty file for writing\n");
	    ...
	    fp = fopen(fileName,"w");		//create an empty file "/tmp/log_RF_switch_1_TraceIndex_1.txt" for writing

	    //printf("debug: open file for write\n");
		...
		return -1;
		    ...
		    send_buf[0] = 0x01;
    		    ret = send(sock,send_buf,1,0);	//send message on a socket. the socket file descriptor is "sock", and the message is stored in the buffer "send_buf",
							//the length of the message is 1 byte.? 
	    }	

	    //printf("debug: count total message\n");
	    ...
	    //printf("debug: waiting for csi from kernel\n");
...
//	    strcat(command," yanbo@155.69.150.195:~/datafolder/openwrt_to_ubuntu/"); 	//change the username and ip
//	    system(command);

	    memset(command,0,sizeof(command));
	    strcpy(command,"cp "); 
	    strcat(command,fileName);
	    strcat(command," ~/yanbo/work/AntSwitch/matlab/wrt2ubuntu/data/.");
	    system(command);
