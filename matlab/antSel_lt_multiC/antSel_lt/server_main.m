...
%                 end
                ...
                if pkt_cbn == 64
                    % save last CSI to buffer
                    csiBuffer.CSI(4,:,:,:) = CSI;
                    csiBuffer.RSSI(4,:) = rssi_v;
                    csiBuffer.cbnIdx(4) = pkt_cbn;
                    
                    if(isempty(find(csiBuffer.RSSI==0,1)))
                        % compute bestCbn
                        csiCbn_struct = antCbn_mapping_2(csiBuffer);
                        bestCbn = bestCbn_select_2(csiCbn_struct);
