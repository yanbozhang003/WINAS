...
for idx = 1:1:seq_len
    ...
    if idx > 1
        esnr_inUse_pred(idx,1) = alpha*esnr_inUse(idx,1)+(1-alpha)*esnr_inUse_pred(idx-1,1);
    end
end
...
%         plot(esnr_inUse_pred,'r.-');hold on
...
subplot(3,1,2)
% plot(esnr_top_pred(1:100,1) - esnr_inUse,'r.-');hold on
plot(V,'k.-');hold off
xlim([0 seq_len]);
ylim([0 150]);
subplot(3,1,3)
