...
effSNR_series.series_s = zeros(8,1); % smoothed value
...
global model;
model = importKerasNetwork('./model/esnr_vs_mcs5_cbn1_96to10.h5');
...
% load('./debugging/data_dbg.mat');
% n_pkt = length(data_dbg);
% 
% for pkt_idx = 1:1:n_pkt
%                 CSI_entry = data_dbg{pkt_idx,1};
                ...
                timestamp   = CSI_entry.timestamp * 10^-6; 
...
% %                 end
                ...
                if state == 0                    
                    ...
                    [len_series,~] = size(w_eSNR_series.eSNR);
                    if len_series > 1
                        pred_eSNR_series = eSNR_series_estimate(w_eSNR_series);
% %                         disp(pred_eSNR_series.eSNR);
                        cbnIdx = systemStateCtrl(pred_eSNR_series);
                        
                        y1 = w_eSNR_series.eSNR(:,8);
% %                         y1 = effSNR_series.series(:,8);
                        y2 = pred_eSNR_series.eSNR(:,8);
% %                         % plot for debugging
% %                         
% %                         subplot(3,2,[2 4 6])
% %                         plot(y1,'k.-');hold on
% %                         plot(length(y1)+1:length(y1)+10,y2,'r.-');hold off
% %                         
% %                         drawnow
                    else
                        cbnIdx = systemStateCtrl(w_eSNR_series);
